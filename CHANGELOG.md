...
- **New `OVERLAYS` catalog in `app/dynbg.py`** — six presets at launch: `noise-grain` (the Hyprlab sandpaper recipe — SVG fractal-noise data-URL at 3% opacity, tiled), `scanlines` (2px horizontal stripes at 1.5% alpha), `linen` (two-direction stripe weave), `vignette` (radial darken from corners), `crosshatch` (diagonal stripe pair), `dot-weave` (tiny halftone lattice). Each has dark-mode parity and respects `prefers-reduced-motion`.
...
- **`overlay_scope` config field** with two values: `all` (default — texture rides over backgrounds AND content, the Hyprlab feel) and `bg` (texture sits beneath cards / typography). The latter applies the `.fe-dynbg-overlay--bg-only` modifier which drops the overlay's `z-index` from 10 to 0 so the host's content (forced to `z-index: 1` by the host's `:where` rule) paints on top.
